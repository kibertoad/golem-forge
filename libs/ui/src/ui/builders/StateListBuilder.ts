...
export class StateListBuilder {
  ...
  build() {
    ...
    for (let count = 0; count < this.entries.length; count++) {
      ...
      const x = this.offsetX ? this.startTextX + count * this.offsetX : this.startTextX
      const y = this.offsetY ? this.startTextY + count * this.offsetY : this.startTextY
      ...
      const valueText = this.scene.add
        .text(labelText.x + labelText.width / 2 + 20, y, entry!.value.value.toString(), {
          fontSize: '20px',
