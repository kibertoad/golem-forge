import type { StateHolder } from '@potato-golem/core'
import * as Phaser from 'phaser'

export interface StateTransition<
  View,
  Model extends StateHolder<StateFlags, MainStates>,
  StateFlags extends string,
  MainStates extends string,
> {
...
export class StateUIManager<
  View extends ViewListener,
  Model extends StateHolder<StateFlags, MainStates>,
  StateFlags extends string,
  MainStates extends string = '',
> {
  ...
  private readonly transitions: Record<
    Triggers,
    StateTransition<View, Model, StateFlags, MainStates>[]
  >
  ...
  addTransition(
    trigger: Triggers,
    transition: StateTransition<View, Model, StateFlags, MainStates>,
  ) {
  ...
  processTransitions(trigger: Triggers) {
    ...
    for (const transition of this.transitions[trigger]) {
      if (
        transition.exclusiveLock &&
        !exclusiveLocks.includes(transition.exclusiveLock) &&
