...
export class StarmapUIScene extends Phaser.Scene {
    ...
    create() {
        ...
        this.encounterBg = this.add.rectangle(0, 0, 560, 350, 0x1a1a28, 0.98)
            .setOrigin(0, 0)
            .setDepth(11001);
        this.encounterImage = this.add.image(40 + 160, 32 + 96, imageRegistry.ENCOUNTER_BACKGROUND)
            .setOrigin(0.5, 0)
            .setDisplaySize(180, 192)
            .setDepth(11002);
        this.encounterTitle = this.add.text(260, 42, "", {
            fontSize: "26px",
            color: "#eaeaff",
            fontStyle: "bold",
            fontFamily: "monospace"
            ...
            .setDepth(11002);
        this.encounterButtonGroup = this.add.container(260, 96)
            .setDepth(11002);

        // Tooltip for encounter option
        this.encounterTooltipText = this.add.text(0, 0, '', {
            fontSize: "16px",
            color: "#fff0da",
            ...
            padding: { x: 12, y: 10 },
            wordWrap: { width: 300 }
        }).setDepth(12000).setVisible(false);
        this.encounterTooltipBg = this.add.rectangle(0, 0, 340, 60, 0x1c1649, 0.96)
            .setOrigin(0, 0)
            .setStrokeStyle(1, 0xffffff)
            .setDepth(11999)
            .setVisible(false);
        // Tooltip is a child of the overlay, so local position!
        this.encounterTooltipBox = this.add.container(0, 0, [this.encounterTooltipBg, this.encounterTooltipText])
            .setDepth(12000)
            .setVisible(false);

        this.encounterOverlayBox = this.add.container(
        ...
        ).setDepth(11000).setVisible(false);
    }
    ...
    showEncounterOverlay(encounter: EncounterModel) {
        ...
        for (const choice of encounter.choices) {
            const show =
                !choice.conditionsToShow ||
                choice.conditionsToShow.every(cond => cond.isSatisfied());
            if (!show) continue;

            const enabled =
                !choice.conditionsToEnable ||
                choice.conditionsToEnable.every(cond => cond.isSatisfied());

            const btn = this.add.text(0, y, choice.name, {
                fontSize: "22px",
                ...
                .on("pointerout", () => {
                    btn.setStyle({ backgroundColor: enabled ? "#444444" : "#222222", color: enabled ? "#fff9c0" : "#aaaabb" });
                    this.hideEncounterTooltip();
                })
                .on("pointerdown", () => {
                    if (enabled) {
    ...
    private centerEncounterOverlay() {
        this.encounterOverlayBox.setPosition(
            this.scale.width / 2 - 280,
            this.scale.height / 2 - 175
        );
    }
    ...
    private showEncounterTooltip(desc: string, btnY: number) {
        ...
        this.encounterTooltipBg.setSize(this.encounterTooltipText.width + 16, this.encounterTooltipText.height + 10);
